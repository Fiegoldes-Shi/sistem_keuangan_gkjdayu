...
if (isset($_SESSION["id_user"]) && !empty($_SESSION["baseurl"])) {
    ...
    header(
        "Location: " . $scheme . "://" . $host . $subdir . "/" . $base . "/",
    );
    exit();
}
...
if (empty($_POST["btnpost"])) {
    ...
    if (empty($_POST["unme"])) {
        ...
        if (empty($_POST["pswd"])) {
            ...
            $un = mysqli_real_escape_string($GLOBALS["conn"], $un);
                ...
                foreach ($aarraytahun as $datatahun) {
                    ...
                    if (isset($datatahun["tanggal_akhir"])) {
                        $_SESSION["tanggal_akhir"] =
                            $datatahun["tanggal_akhir"];
                    } else {
                        $_SESSION["tanggal_akhir"] =
                            "Tanggal akhir tidak tersedia";
                    }
                }
            }
        }
...
if ($gu == 0 and $gp == 1) {
    ...
    $alert = empty($_POST["btnpost"]) ? "" : "Username & Password tidak boleh kosong !";
    ...
    if ($_SESSION["aktif"] == 0) {
        ...
        $scheme =
            !empty($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] !== "off"
                ? "https"
                : "http";
        $host = $_SERVER["HTTP_HOST"]; // includes port, e.g. localhost:8000
        $subdir = rtrim(dirname($_SERVER["SCRIPT_NAME"]), "/");
        header(
            "Location: " .
                $scheme .
                ...
                <div class="card-body">
                    <form action="" method="post">
                        ...
                        <div class="input-group mb-3" style="width:80%; margin-top: 50px;">
                            ...
                            <input type="text" class="form-control" placeholder="Username" aria-label="username" name="unme" aria-describedby="basic-addon1" style="border-top-right-radius: 25px; border-bottom-right-radius: 25px;" required>
                        </div>
                        <p></p>
                        <div class="input-group mb-3" style="width:80%">
                            ...
                            <input type="password" class="form-control" placeholder="Password" aria-label="password" name="pswd" aria-describedby="basic-addon1" style="border-top-right-radius: 25px; border-bottom-right-radius: 25px;" required>
                        </div>
                        <p></p>
                        ...
                        <p class="text-danger text-center small">
                            <?php echo $alert;
// Changed to use echo for clarity
?>
                        </p>
                    </form>
                </div>
    ...
    <script>
        ...
        // Cegah halaman login diload dari Back-Forward Cache (BFCache) browser
        // event.persisted = true artinya halaman diambil dari cache RAM, bukan dari server
        window.addEventListener('pageshow', function(event) {
            if (event.persisted) {
                // Paksa reload dari server agar PHP session check dijalankan ulang

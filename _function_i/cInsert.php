...
class cInsert
{
    function vInsertData($afields, $atable, $avalues)
    {
        ...
        for ($i = 0; $i <= $countarray_field; $i++) {
            ...
            if (
                is_string($avalues[$i]) &&
                preg_match('/^\'(.*)\'$/s', $avalues[$i], $matches)
            ) {
            ...
            } elseif ($avalues[$i] === null || $avalues[$i] === "") {
    ...
    function fInsertDataSilent($afields, $atable, $avalues)
    {
        $countarray_field = count($afields) - 1;
        $fieldname = "";
        $datavalue = "";
    ...
    public function InsertDataArray($afields, $atable, $avalues, $url)
    {
        ...
        foreach ($avalues as $val) {
            if (
                ...
                $escaped_values[] =
                    "'" .
                    mysqli_real_escape_string($GLOBALS["conn"], $matches[1]) .
                    "'";
            } elseif ($val === null || $val === "") {
                $escaped_values[] = "NULL";
                ...
                $escaped_values[] =
                    "'" .
                    mysqli_real_escape_string($GLOBALS["conn"], $val) .
                    "'";
            }
        }
        $valuestring = implode(", ", $escaped_values);

        $sql = "INSERT INTO $atable ($fieldname) VALUES ($valuestring)";

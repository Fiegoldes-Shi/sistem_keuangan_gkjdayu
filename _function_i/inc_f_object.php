...
function _myobject(
   ...
   foreach ($arraypilihan as $datapilihan) {
       ...
       ?>
         ...
         $x ?>" name="<?= $name ?>" type="radio" value="<?= $datapilihan[
    "field1"
] ?>" <?= $checked . " " . $required_attr ?>>
					<label class="form-check-label" for="inlineCheckbox1"><?= $datapilihan[
         "field2"
     ] ?></label>
				</div>
			<?php
   ...
   foreach ($arraypilihan as $datapilihan) {
       ...
       ?>
				<div class="form-check form-check-inline">
					<input class="form-check-input" id="<?= $id ?>" name="<?= $name ?>" type="radio" id="inlineCheckbox1" value="<?= $datapilihan[
    "field2"
] ?>" <?= $checked . " " . $required ?>>
					<label class="form-check-label" for="inlineCheckbox1"><?= $datapilihan[
         "field1"
   ...
   ?>
		...
		<?php break;case 5: //select combo required
			...
			<select name="<?php echo $name; ?>" class="<?php echo $class; ?>" id="<?php echo $id; ?>" <?= $required ?>>
				<?php for ($i = 0; $i <= $x; $i++) {
        if ($data[$i][0] == $value) { ?>
						<option value="<?php echo $data[$i][0]; ?>" selected><?php echo $data[
    $i
][1]; ?></option>
					<?php } else { ?>
...
function _CreateWindowModal(
    ...
    $clsbtn = "btn btn-primary btn-sm";
    ?>
	<button type="button" class="<?php echo $clsbtn; ?>" data-toggle="modal" data-target="<?php echo "#" .
    $idmodal; ?>">
				...
				<div class="modal-content">
					<div class="modal-header">
						<blockquote class="blockquote">
							...
							<?php for ($baris = 0; $baris <= $footer_field; $baris++) {
                  ...
                  switch ($afield[$i][3]) {
                      ...
                      case 13:
                          ...
                          echo '<label for="caption' .
                              ...
                              '">' .
                              strtoupper($afield[$i][0]) .
                              "</label>"; //_myobject($object,$name,$class,$id,$value,$caption,$maxlength,$size,$rows,$cols,$required,$placeholder,$disabled)
                          _myobject(
                              $afield[$i][3],
                              $afield[$i][1],
                              "form-control",
...
function _CreateWindowModalDetil(
    $number,
    $type,
    $name,
    $button,
    ...
    $countcolum = count($titledetil); // get width
    if (empty($width)) {
        $modalsize = "";
    } elseif ($width == "xl") {
        $modalsize = "modal-xl";
...
function _CreateWindowModalPencairan(
    $number,
    $type,
    $name,
    $button,
	...
	<?php // Cek apakah tombol disabled atau tidak


 $disabledAttr = $disabled ? "disabled" : "";
 $btnClass = $disabled ? "btn-secondary" : "btn-primary";
 // Warna abu-abu jika disabled
    ?>
...
function _CreateWindowModalUpload(
    $number,
    $type,
    $name,
    $button,
...
// create window
function _CreateWindowModalAdd(
    $number,
    $type,
    $name,
    $button,
...
function _CreateWindowModalUpdate(
    $number,
    $type,
    $name,
    $button,
		...
		<div class="modal-dialog <?= $modalsize ?> text-start">
          ...
          switch ($afield[$i][3]) {
              ...
              case 17:
                  ...
                  break;
              case 19:
                  // textbox text required (validasi huruf dan spasi)
                  _myobject(
                      $afield[$i][3],
                      $afield[$i][1],
                      "form-control",
...
function _CreateWindowModalStatus(
    $number,
    $type,
    $name,
    $button,
...
<?php #function _CreateWindowModalDelete($number, $type, $name, $button, $width, $height, $title, $acaption, $value, $linkurl, $disabled = false)


function _CreateWindowModalDelete(
    $number,
    $type,
    $name,
    $button,
    ...
    $number = $type . $name . $number; // get width
    if (empty($width)) {
        $modalsize = "";
    } elseif ($width == "xl") {
        $modalsize = "modal-xl";
          ...
          for ($k = 1; $k < $countcolum; $k++) { ?>
				...
				<form method="post" action="<?= $linkurl ?>">
					<div class="modal-body">
						...
						<?php for ($k = 0; $k <= 2; $k++) {
          echo '<input type="hidden" name="hiddendeletevalue' .
              $k .
              '" value="' .
              $value[0][$k] .
              '">';
...
function _CreateWindowModalValid(
    $number,
    $type,
    $name,
    $button,
          ...
          for ($k = 1; $k < $countcolum; $k++) { ?>
				...
				<form method="post" action="<?= $linkurl ?>">
					<div class="modal-body">
						<h6 class=""><?= $caption ?> </h6>
						<?php for ($k = 0; $k <= 2; $k++) {
          echo '<input type="hidden" name="hiddenupdatevalue' .
              $k .
              '" value="' .
              $value[0][$k] .
              '">';
...
function _CreateWindowModalReset(
    $number,
    $type,
    $name,
    $button,
          ...
          for ($k = 1; $k < $countcolum; $k++) { ?>
				...
				<form method="post" action="<?= $linkurl ?>">
					<div class="modal-body">
						...
						<?php for ($k = 0; $k <= 2; $k++) {
          echo '<input type="hidden" name="hiddenresetvalue' .
              $k .
              '" value="' .
              $value[0][$k] .
              '">';
...
function _CreateWindowModalProses(
    $number,
    $type,
    $name,
    $button,
...
function _CreateWindowModalViewFile(
    $number,
    $type,
    $name,
    $button,
...
function UploadTruePdf($field, $path)
{
    ...
    $status_all = [$field, $statustrue, $path, $docfilenewname, $docfilename];
    return $status_all;
}
function _createpage($l, $range, $page, $table, $rpp)
{
    $res = "SELECT count(*) as recnumber FROM " . $table . " "; //echo $res."<br>";
    $view = new cView();
    $view->vViewData($res);
    ...
    if ($maxrows > $rpp) {
        ...
        echo '<li><a href="?l=' .
            $l .
            '&pg=1" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
...
function _BackButton($url)
{
    ...
    echo '<li class="page-item"><a class="page-link" href="' .
        $url .
        '" style="border-radius:30px; border:0.5px solid #c0c0c0">
...
function _CreateNavsData($sql, $type)
{
    ...
    $safe_l = isset($_GET["l"])
        ? htmlspecialchars($_GET["l"], ENT_QUOTES, "UTF-8")
        : "";
    ?>
	<ul class="nav nav-tabs">
      ...
      $cactive = "";
		...
		<?php if (empty($type)) { ?>
			<li role="presentation" class="<?php echo $cactive; ?>"><a href="?l=<?php echo $safe_l; ?>&id=0">Semua</a></li>
		<?php } ?>
		<?php foreach ($arraysql as $datasql) {
      if ($safe_id == $datasql["field1"]) {
          $cactive = "active";
      } else {
          $cactive = "";
      } ?>
			<li role="presentation" class="<?php echo $cactive; ?>"><a href="?l=<?php echo $safe_l; ?>&id=<?php echo htmlspecialchars(
    $datasql["field1"],
    ENT_QUOTES,
    "UTF-8",
...
function UploadTrue($field, $path)
{
    ...
    $statustrue = $status_upload . $status_size;
    $status_all = [$field, $statustrue, $path, $docfilenewname];
    return $status_all;
}
...
<?php echo '<button id="' .
    $id .
    '" onclick="reply_click(this.id)" class="btn btn-info" data-toggle="modal" data-target="' .
    $name .
    '" data-placement="bottom" title="" value="del"><span class="glyphicon glyphicon-download"></span></button>';
}
...
{
    $sql =
        "SELECT a.kd_coa_dep, sum(a.nominal) nominalpermohoan 
...
{
    $sql =
        "SELECT a.idpengajuan, count(a.idpengajuan) nCair 
...
{
    $sql =
        "SELECT a.kodestatus, b.keterangan ketaju, b.warna, COUNT(a.idpengajuan) nAju
	FROM tra_pengajuan_detil a
...
function _GetBulanTahun($value)
{
    ...
    $nmb = "";
    $sql =
        "SELECT a.kodebulan, a.namabulan FROM ref_bulan a WHERE a.kodebulan ='" .
        $bln .
        "'";
    $view = new cView();
    $arrayview = $view->vViewData($sql);

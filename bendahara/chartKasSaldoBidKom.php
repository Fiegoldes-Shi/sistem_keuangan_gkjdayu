...
$tahun =
    isset($_GET["tahun"]) && !empty($_GET["tahun"])
        ? intval($_GET["tahun"])
        : date("Y");
...
if ($idBidang) {
    $komisiList = fetchAll(
        $conn,
        "SELECT * FROM komisi WHERE id_bidang = ?",
        "i",
        [$idBidang],
    );
    if ($komisiList !== false) {
...
if ($idKomisi) {
    ...
    $sqlPenerimaan = "
        ...
        ORDER BY bulan ASC
    ";
    $penerimaanData = fetchAll($conn, $sqlPenerimaan, "iiii", [
        $idKomisi,
        $tahun,
        $startMonth,
        $endMonth,
    ...
    $sqlPengeluaran = "
        ...
        ORDER BY bulan ASC
    ";
    $pengeluaranData = fetchAll($conn, $sqlPengeluaran, "iiii", [
        $idKomisi,
        $tahun,
        $startMonth,
        $endMonth,
    ...
    for ($bulan = $startMonth; $bulan <= $endMonth; $bulan++) {
        ...
        $saldoGereja +=
            $penerimaanBulan["dana_gereja"] - $pengeluaranBulan["dana_gereja"];
        $saldoSwadaya +=
            $penerimaanBulan["dana_swadaya"] -
            $pengeluaranBulan["dana_swadaya"];

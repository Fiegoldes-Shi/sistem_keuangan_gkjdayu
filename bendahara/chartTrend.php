...
if (!$fiskalData || count($fiskalData) < 2) {
    echo json_encode([
        "error" =>
            "Tidak cukup data fiskal untuk prediksi. Minimal 2 tahun terakhir diperlukan.",
    ]);
    exit();
}
...
if (count($idFiskalHistoris) < 2) {
    echo json_encode([
        "error" => "Minimal 2 tahun historis dibutuhkan untuk prediksi.",
    ]);
    exit();
}
...
$predictedPenerimaan = round(
    $penerimaanReg["slope"] * $tahunPrediksi + $penerimaanReg["intercept"],
);
$predictedPengeluaran = round(
    $pengeluaranReg["slope"] * $tahunPrediksi + $pengeluaranReg["intercept"],
);
...
$penerimaanNearTarget =
    abs($predictedPenerimaan - $lastPenerimaan) / max($predictedPenerimaan, 1) <
    0.1;
$pengeluaranNearTarget =
    abs($predictedPengeluaran - $lastPengeluaran) /
        max($predictedPengeluaran, 1) <
    0.1;

$persentasePenerimaan = round(
    ($lastPenerimaan / max($predictedPenerimaan, 1)) * 100,
    2,
);
$persentasePengeluaran = round(
    ($lastPengeluaran / max($predictedPengeluaran, 1)) * 100,
    2,
);
...
$result = [
    ...
    "penerimaan" => array_map(
        function ($tahun, $nominal) {
            return ["tahun" => $tahun, "nominal" => $nominal];
        },
        $years,
        $penerimaanData,
    ),
    "pengeluaran" => array_map(
        function ($tahun, $nominal) {
            return ["tahun" => $tahun, "nominal" => $nominal];
        },
        $years,
    ...
    "model_regresi" => [
        "penerimaan" =>
            "y = " .
            round($penerimaanReg["slope"], 2) .
            "x + " .
            round($penerimaanReg["intercept"], 2),
        "pengeluaran" =>
            "y = " .
            round($pengeluaranReg["slope"], 2) .
            "x + " .
            round($pengeluaranReg["intercept"], 2),
    ],
    "model_regresi_nilai" => [

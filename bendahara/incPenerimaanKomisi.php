...
if (!empty($_POST["btn_bulk_validasi"])) {
    $sql_bulk = "SELECT id_penerimaan FROM realisasi_penerimaan_komisi WHERE id_fiskal = $id_fiskal AND status = 'Belum Tervalidasi'";
    ...
    <div class="section filter-section" style="padding: 20px; margin-bottom: 20px; box-sizing: border-box; width: 100%;">
        <form action="" method="post" autocomplete="off" style="margin: 0;">
                <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
                    ...
                    <button type="submit" name="btn_bulk_validasi" value="1" style="background-color:#2e8b57; color:white; border-radius:4px; border:none; height: 38px; padding: 0 15px; cursor:pointer; white-space: nowrap;" onclick="return confirm('Validasi semua transaksi penerimaan komisi yang belum tervalidasi?');">✔ Validasi Semua</button>
                    <?php endif; ?>
                </div>
            </form>
    </div>

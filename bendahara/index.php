...
session_start();
// Reset session filter on GET request to ensure clean state
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    foreach ($_SESSION as $key => $value) {
        if (strpos($key, "bidang_") === 0 || strpos($key, "komisi_") === 0) {
            unset($_SESSION[$key]);
        }
    }
}
...
// Cegah browser cache halaman dashboard
...
$tahun_aktif = isset($_SESSION["tahun_aktif"])
    ? $_SESSION["tahun_aktif"]
    : null;
...
if ($tahun_aktif) {
    ...
    if (!empty($array)) {
        ...
        $sql_latest =
            "SELECT id_fiskal, tahun, status_aktif FROM fiskal WHERE status_aktif = 1 ORDER BY tahun DESC LIMIT 1";
...
<body>
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item dropdown dropdown-menu-end" style="display: flex; align-items: center;">
                        ...
                        <form method="POST" action="" style="display: flex; align-items: center;">
                            <select name="tahun" id="selected_year" onchange="this.form.submit()"
                                style=" border: none; background: none; font-size: inherit; cursor: pointer; color: #0870B6; font-weight: bold;">
                                <?php
                                $sql =
                                    "SELECT DISTINCT tahun FROM fiskal ORDER BY tahun ASC";
                                $result = $GLOBALS["conn"]->query($sql);
                                while ($row = $result->fetch_assoc()) {
                                    $selected =
                                        $tahun_aktif == $row["tahun"]
                                            ? "selected"
                                            : "";
                                    echo "<option value='" .
                                        $row["tahun"] .
                    ...
                    <li class="nav-item dropdown dropdown-menu-end">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li><a class="dropdown-item" href="#"><?= $_SESSION[
                                "nama"
                            ] ?></a></li>
                            <li>
                                <hr class="dropdown-divider">
    ...
    $posBendahara = array_search("bendahara", $segments); // deprecated
    $last_segment = end($segments);
    $link = is_numeric($last_segment) ? (int) $last_segment : 0;
 ...
 <!-- jQuery -->
    ...
    <!-- Hint format Rupiah pada input nominal uang -->
    <script src="../js/rupiah-hint.js?v=1.1"></script>
    ...
    <script>
        // Cegah halaman dashboard diload dari Back-Forward Cache (BFCache) browser

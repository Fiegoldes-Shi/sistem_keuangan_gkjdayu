...
if ($bidang > 0) {
    $res = mysqli_query(
        $db,
        "SELECT nama_bidang FROM bidang WHERE id_bidang = $bidang",
    );
...
if ($komisi > 0) {
    $res = mysqli_query(
        $db,
        "SELECT nama_komisi FROM komisi WHERE id_komisi = $komisi",
    );
    if ($row = mysqli_fetch_assoc($res)) {
        $namaKomisi = $row["nama_komisi"];
    }
}
...
if ($bidang > 0) {
    $filter .= " AND k.id_bidang = $bidang";
}
if ($komisi > 0) {
    $filter .= " AND k.id_komisi = $komisi";
}
...
if ($bidang > 0) {
    $filterG .= " AND k.id_bidang = $bidang";
}
if ($komisi > 0) {
    $filterG .= " AND k.id_komisi = $komisi";
}
...
if (!$resultP) {
    die("Query Penerimaan Gagal: " . mysqli_error($db));
}

$resultG = mysqli_query($db, $queryPengeluaran);
if (!$resultG) {
    die("Query Pengeluaran Gagal: " . mysqli_error($db));
}
...
if (empty($data)) {
    ...
    foreach ($data as $row) {
        ...
        $jumlah_pengeluaran =
            $row["tipe"] === "pengeluaran" ? $row["jumlah"] : 0;
        ...
        echo "<tr>
            ...
            <td>" .
            ($jumlah_penerimaan > 0
                ? "Rp " . number_format($jumlah_penerimaan, 0, ",", ".")
                : "-") .
            "</td>
            <td>" .
    ...
    echo "<tr style='font-weight:bold;'>
        ...
        <td>Rp " .
        number_format($total_dg_penerimaan, 0, ",", ".") .
        "</td>
        <td>Rp " .
        number_format($total_ds_penerimaan, 0, ",", ".") .
        "</td>
        <td colspan='2'>Rp " .
        number_format($total_penerimaan, 0, ",", ".") .
        "</td>
    </tr>";
    ...
    echo "<tr style='font-weight:bold;'>
        ...
        <td>Rp " .
        number_format($total_dg_pengeluaran, 0, ",", ".") .
        "</td>
        <td>Rp " .
        number_format($total_ds_pengeluaran, 0, ",", ".") .
        "</td>
        <td colspan='2'>Rp " .
    ...
    echo "<tr style='font-weight:bold;'>
        ...
        <td>Rp " .
        number_format($saldo_dg, 0, ",", ".") .
        "</td>
        <td>Rp " .
        number_format($saldo_ds, 0, ",", ".") .
        "</td>
        <td colspan='2'>Rp " .

...
$tahun =
    isset($_GET["tahun_aktif"]) && !empty($_GET["tahun_aktif"])
        ? intval($_GET["tahun_aktif"])
        : $_SESSION["tahun_aktif"] ?? date("Y");

// Ambil bulan dari GET
$bulan =
    isset($_GET["bulan"]) && is_numeric($_GET["bulan"]) && $_GET["bulan"] !== ""
        ? intval($_GET["bulan"])
        : null;
...
echo "<tr><th colspan='7'>Periode: Tahun $tahun" .
    ($bulan
        ? " - Bulan " . ($nama_bulan[$bulan] ?? "Tidak Diketahui")

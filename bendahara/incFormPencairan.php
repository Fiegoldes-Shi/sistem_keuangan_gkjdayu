...
if (isset($_POST["save"])) {
    ...
    if (!empty($_FILES["bukti_transfer"]["name"])) {
        $uploadDir =
            realpath("../uploads/bukti_transfer/") . DIRECTORY_SEPARATOR;
    ...
    $inputData = [
        ...
        "biaya_tf" =>
            isset($_POST["biaya_tf"]) && $_POST["biaya_tf"] !== ""
                ? $_POST["biaya_tf"]
                : 0,
        "jumlah" => $_POST["jumlah"],
...
if (isset($_POST["submit_all"])) {
    if (
        isset($_SESSION["temp_data15"]) &&
        count($_SESSION["temp_data15"]) > 0
    ) {
        ...
        foreach ($_SESSION["temp_data15"] as $data) {
            ...
            if (!$insert->fInsertDataSilent($datafield, "pencairan", $datavalue)) {
                $success = false;
                break;
            }

            $datafield_penerimaanKomisi = [
                "id_fiskal",
                "id_program",
                "id_user",
                "id_bidang",
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-12">
      <?php _myHeader("newspaper", "Pencairan", "Entri Data"); ?>
    </div>
  </div>
  ...
  <div class="section">
    <div class="horizontal" style="height: 90px;">
      <div class="form-group" style="width:100%; margin-left:30px;">
        <label for="tanggal_pencairan" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Tanggal Pencairan</label>
        <input style="width:95%" type="date" id="tanggal_pencairan" name="tanggal_pencairan" placeholder="" required min="<?= $tahun_aktif -
            1 ?>-01-01" max="<?= $tahun_aktif ?>-12-31" value="<?= isset(
    $_SESSION["edit_data"],
)
    ? $_SESSION["edit_data"]["tanggal_pencairan"]
    : "" ?>" />
    ...
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      <div class="form-group" style="width:100%; margin-left: 30px;">
        ...
        <select style="width:90%" id="program" name="program" required>
          ...
          if (isset($_SESSION["komisi_" . $link])) {
              $query =
                  "SELECT program.*, komisi.nama_komisi, bidang.nama_bidang, fiskal.tahun
                      FROM program
                      ...
                      WHERE fiskal.tahun = $tahun_aktif AND program.id_komisi = " .
              ...
              $query =
                  "SELECT program.*, bidang.nama_bidang, fiskal.tahun
                      FROM program
                      ...
                      WHERE fiskal.tahun = $tahun_aktif AND program.id_bidang = " .
                  $_SESSION["bidang_" . $link] .
                  "
                      AND ( pengajuan.status = 'Disetujui' OR (
                      pengajuan.status = 'Disetujui dan Dana telah Cair'
      ...
      <div class="form-group" style="width:100%">
        ...
        <select style="width:90%" id="akun" name="akun" required>
          ...
          <?php
          $sql =
              "SELECT id_akun, nama_akun FROM akun WHERE jenis_debitKredit = 'Debet' AND status_input = 1 ORDER BY kode_akun ASC";
          $result = $conn->query($sql);
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  $selected = "";
    ...
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      <div class="form-group" style="width:45%; margin-left:30px">
        <label for="jumlah" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Jumlah Pencairan</label>
        <input style="width:90%" type="number" id="jumlah" name="jumlah" placeholder="Masukkan Jumlah Pencairan" value="<?= isset(
            $_SESSION["edit_data"],
        )
      ...
      <div class="form-group" style="width:45%; margin-left:-40px">
        <label for="biaya_tf" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Biaya Admin</label>
        <input style="width:89%" type="number" id="biaya_tf" name="biaya_tf" placeholder="Jumlah Biaya Admin Transfer" value="<?= isset(
            $_SESSION["edit_data"],
        )
            ? $_SESSION["edit_data"]["biaya_tf"]
            : "" ?>" min="1" />
      </div>
      <div class="form-group" style="width:100%; margin-left:50px">
        <label for="bank" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Bank Penerima</label>
        <select style="width:90%" id="bank" name="bank">
          ...
          if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                  $selected =
                      isset($_SESSION["edit_data"]) &&
                      $_SESSION["edit_data"]["bank"] == $row["id_bank"]
                          ? "selected"
                          : "";
                  echo "<option value='" .
                      $row["id_bank"] .
    ...
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      <div class="form-group" style=" width:100%; margin-left: 30px;">
        <label for="keterangan" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Keterangan</label>
        <input style="width:90%" type="text" id="keterangan" name="keterangan" placeholder="Masukkan Keterangan" value="<?= isset(
            $_SESSION["edit_data"],
        )
            ? $_SESSION["edit_data"]["keterangan"]
            : "" ?>" required />
      </div>
      <div class="form-group" style="height:110px; width:100%;">
        <label for="bukti_transfer" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Bukti Transfer/Pencairan</label>
        ...
        <tbody>
          ...
          if (
              ...
              <tr class=''>
                ...
                <td><?= date(
                    "d-M-Y",
                    strtotime($data["tanggal_pencairan"]),
                ) ?></td>
                <td><?= $programNama ?></td>
                <td><?= $akunNama ?></td>

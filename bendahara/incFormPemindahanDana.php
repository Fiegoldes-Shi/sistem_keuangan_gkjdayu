...
if (isset($_POST["submit_all"])) {
    if (
        isset($_SESSION["temp_data24"]) &&
        count($_SESSION["temp_data24"]) > 0
    ) {
  ...
  <div class="section">
    <form action="" method="post" enctype="multipart/form-data">
      <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
        <div class="form-group" style="width:100%; margin-left: 30px; ">
          <label for="tanggal" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Tanggal Pemindahan</label>
    ...
    : "" ?>" required />
        ...
        <div class="form-group" style="width: 100%">
          <label for="keterangan" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Keterangan Pengeluaran</label>
          <input style="width:77%" type="text" id="keterangan" name="keterangan" placeholder="Masukkan Keterangan Pengeluaran" value="<?= isset(
              $_SESSION["edit_data"],
          )
      ...
      <div class="horizontal" style="height:80px">
        <div class="form-group" style="width:40%; margin-left: 30px ">
          <label for="akun_sumber" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Sumber Akun</label>
          <select style="width:90%" id="akun_sumber" name="akun_sumber" required>
            <option value="">-- Pilih Akun --</option>
            <?php
            $sql =
                "SELECT id_akun, nama_akun FROM akun where jenis_debitKredit = 'Debet' ORDER BY kode_akun";
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    if (
                        ...
                        $_SESSION["edit_data"]["akun_sumber"] == $row["id_akun"]
                    ) {
                        $selected = "selected";
                    }
                    echo "<option value='" .
        ...
        <div class="form-group" style="width:20%; margin-left: -70px ">
          <label for="jumlah" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Jumlah Pemindahan(Rp)</label>
          <input style="width:90%;" type="number" id="jumlah" name="jumlah" placeholder="Jumlah" value="<?= isset(
              $_SESSION["edit_data"],
          )
        ...
        <div class="horizontal" style="height:80px">
          <div class="form-group" style="width: 120%; margin-left: -40px">
            <label for="akun_tujuan" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Akun Tujuan</label>
            <select style="width:90%" id="akun_tujuan" name="akun_tujuan" required>
              <option value="">-- Pilih Akun --</option>
              <?php
              $sql =
                  "SELECT id_akun, nama_akun FROM akun where jenis_debitKredit = 'Kredit' ORDER BY kode_akun";
              $result = $conn->query($sql);
              if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                      $selected = "";
                      if (
                          isset($_SESSION["edit_data"]) &&
                          $_SESSION["edit_data"]["akun_tujuan"] ==
                              $row["id_akun"]
                      ) {
                          $selected = "selected";
                      }
        ...
        <tbody>
          ...
          if (
              ...
              foreach ($_SESSION["temp_data24"] as $index => $data) {
                  ...
                  $cnourut = $cnourut + 1;
                  $akunSumber = !empty($data["akun_sumber"])
                      ? getNameFromId(
                          "akun",
                          "id_akun",
                          "nama_akun",
                      ...
                      : "-";
                  $akunTujuan = !empty($data["akun_tujuan"])
                      ? getNameFromId(
                          "akun",
                          "id_akun",
                          "nama_akun",
          ...
          <tr>
            ...
            <td style="color:#483d8b; font-weight:bolder" class="text-end"><?= number_format(
                $total,
                0,
                ",",
                ".",

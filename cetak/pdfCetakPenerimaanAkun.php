...
if (isset($_SESSION["bulan"])) {
    ...
    $title =
        "Rekapitulasi_Penerimaan_Gereja_" .
        $nama_bulan[$bulan] .
        "_" .
        $tahun_aktif;
} else {
...
if (isset($_SESSION["id_user"])) {
    ...
    $result = mysqli_query($GLOBALS["conn"], $sql); // Cek apakah ada data
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $jumlah_realisasi =
        $row["jumlah_realisasi"] == 0 || empty($row["jumlah_realisasi"])
            ? "-"
            : number_format($row["jumlah_realisasi"], 0, ",", ".");
...
$pdf->AddPage(); // set auto page breaks

...
while ($row = mysqli_fetch_assoc($result_data)) {
    ...
    $dataGrouped[$jenis]["total_pengeluaran"] += $pengeluaran; // Tambahkan ke total per bulan dari semua jenis_akun
...
foreach ($dataGrouped as $jenisAkun => $data) {
    ...
    $saldo_akhir =
        $saldo_awal + $data["total_penerimaan"] - $data["total_pengeluaran"]; // Akumulasi total
    ...
    $dataall =
        ...
        htmlspecialchars($jenisAkun) .
        '</td>
                            <td style="text-align: center; font-weight: bold;">Penerimaan</td>
                            <td rowspan="2" style="text-align: right; vertical-align: middle; font-weight: bold;">' .
    ...
    $dataall .=
        ...
        number_format($data["total_penerimaan"], 0, ",", ".") .
        '</td>
                            <td rowspan="2" style="text-align: right; font-weight: bold;">' .
...
for ($b = 1; $b <= 12; $b++) {
    ...
    $dataall .=
        '<td style="text-align: right; font-weight: bold;">' .
        number_format($totalPenerimaanPerBulan, 0, ",", ".") .
        "</td>";
}
$dataall .=
    '<td  style="text-align: right; font-weight: bold;">' .
    number_format($totalPenerimaanSemua, 0, ",", ".") .
    '</td>
                <td rowspan="2" style="text-align: right; font-weight: bold;">' .
...
for ($b = 1; $b <= 12; $b++) {
    ...
    $dataall .=
        '<td style="text-align: right; font-weight: bold;">' .
        number_format($totalPengeluaranPerBulan, 0, ",", ".") .
        "</td>";
}
$dataall .=
    '<td style="text-align: right; vertical-align: middle;  font-weight: bold;">' .
    number_format($totalPengeluaranSemua, 0, ",", ".") .
    '</td>
                </tr>';
$tabelfooter = "</table>";
$date = date("d-m-Y");
...
// $pdf->AddPage('L');
$pdf->AddPage("L", [400, 210]); // set auto page breaks

...
if (isset($_SESSION["bulan"])) {
    ...
    $title =
        "Rekapitulasi_Pengeluaran_Gereja_" .
        $nama_bulan[$bulan] .
        "_" .
        $tahun_aktif;
} else {
...
if (!empty($bulan)) {
    ...
    if ($tahun == 0) {
        ...
        $sql .=
            "WHERE a.jenis_debitKredit = 'Debet' AND a.statusAktif = 1 AND f.tahun = " .
            $tahun .
            " GROUP BY f.tahun, a.kode_akun";
        $laporan = "TAHUN " . $tahun;
    }
...
$total_danaSwadaya = 0;
$total_rencana = 0;
$total_realisasi = 0; // $saldo = $saldo_awal;
while ($row = mysqli_fetch_assoc($result)) {
    $dana_gereja =
        $row["dana_gereja_rencana"] == 0 || empty($row["dana_gereja_rencana"])
            ? "-"
            : number_format($row["dana_gereja_rencana"], 0, ",", ".");
    $dana_swadaya =
        $row["dana_swadaya_rencana"] == 0 || empty($row["dana_swadaya_rencana"])
            ? "-"
            : number_format($row["dana_swadaya_rencana"], 0, ",", ".");
    $rencana =
        $row["jumlah_rencana"] == 0 || empty($row["jumlah_rencana"])
            ? "-"
            : number_format($row["jumlah_rencana"], 0, ",", ".");
    $realisasi =
        $row["dana_gereja_realisasi"] == 0 ||
        empty($row["dana_gereja_realisasi"])
            ? "-"
            : number_format($row["dana_gereja_realisasi"], 0, ",", ".");
    $total_danaGereja = $total_danaGereja + $row["dana_gereja_rencana"];
    $total_danaSwadaya = $total_danaSwadaya + $row["dana_swadaya_rencana"];
    $total_rencana =
        $total_rencana +
        $row["dana_gereja_rencana"] -
        $row["dana_swadaya_rencana"];

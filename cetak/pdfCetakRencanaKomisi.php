...
class MYPDF extends TCPDF //Page header
{
    public function Header()
    {
        ...
        $this->SetFont("helvetica", "B", 20); // Title
        $this->Cell(
            0,
            15,
            "<< TCPDF Example 003 >>",
    ...
    public function Footer()
    {
        ...
        $this->SetY(-15); // Set font
        $this->SetFont("helvetica", "I", 8); // Page number
        $this->Cell(
            0,
            10,
...
require_once "../_function_i/cConnect.php";
$myConn = new cConnect();
$conn = $myConn->goConnect();
if (isset($_SESSION["id_user"])) {
...
if (isset($_SESSION["komisi"]) || isset($_SESSION["bidang"])) {
    if (isset($_SESSION["komisi"])) {
        $sql_penerimaan =
            "SELECT * from v_rencanakomisi WHERE jenis COLLATE utf8mb4_general_ci='Rencana Penerimaan' AND id_komisi =" .
            $id_komisi .
            "";
        $sql_pengeluaran =
            "SELECT * from v_rencanakomisi WHERE jenis COLLATE utf8mb4_general_ci='Rencana Pengeluaran' AND id_komisi =" .
            $id_komisi .
            "";
    } elseif (isset($_SESSION["bidang"])) {
        $sql_penerimaan =
            "SELECT * from v_rencanakomisi WHERE jenis COLLATE utf8mb4_general_ci='Rencana Penerimaan' AND id_bidang =" .
            $id_bidang .
            "";
        $sql_pengeluaran =
            "SELECT * from v_rencanakomisi WHERE jenis COLLATE utf8mb4_general_ci='Rencana Pengeluaran' AND id_bidang =" .
            $id_bidang .
            "";
    }
    if (!empty($tahun)) {
    ...
    foreach ($data_per_program as $program => $dataList) {
        ...
        foreach ($dataList as $row) {
            $number++;
            $volume =
                $row["volume"] == 0 || empty($row["volume"])
                    ? "-"
                    : number_format($row["volume"], 0, ",", ".");
            $harga_satuan =
                $row["harga_satuan"] == 0 || empty($row["harga_satuan"])
                    ? "-"
                    : number_format($row["harga_satuan"], 0, ",", ".");
            $jumlah =
                $row["jumlah"] == 0 || empty($row["jumlah"])
                    ? "-"
                    : number_format($row["jumlah"], 0, ",", ".");
            $dana_gereja =
                $row["dana_gereja"] == 0 || empty($row["dana_gereja"])
                    ? "-"
                    : number_format($row["dana_gereja"], 0, ",", ".");
            $dana_swadaya =
                $row["dana_swadaya"] == 0 || empty($row["dana_swadaya"])
                    ? "-"
                    : number_format($row["dana_swadaya"], 0, ",", ".");
        ...
        $dataall1 .=
            ...
            '</td>
                        <td>' .
            number_format($total_danaGereja, 0, ",", ".") .
            '</td>
                        <td>' .
            number_format($total_danaSwadaya, 0, ",", ".") .
            '</td>
                        <td>' .
    ...
    $tabelfooter1 =
        '<tr style="text-align: right; font-weight:bold; background-color: #d9edf7;">
    ...
    $allshowdata2 =
        $tabelheader . $tabelcaption . $dataall . $tabelfooter . $user;
    $pdf = new TCPDF();
    $pdf->SetCreator(PDF_CREATOR);
    $pdf->SetAuthor("Nama Anda");
    $pdf->SetTitle("Laporan Rencana Komisi");
    $pdf->setFooterFont([PDF_FONT_NAME_DATA, "", PDF_FONT_SIZE_DATA]);
    $pdf->SetFooterMargin(PDF_MARGIN_FOOTER); // Start First Page Group
    $pdf->startPageGroup(); // Tambahkan halaman
    $pdf->AddPage("L"); // set auto page breaks
    $pdf->SetAutoPageBreak(true, PDF_MARGIN_BOTTOM); // Tambah Gambar
    ...
    $pdf->Cell(
        0,
        10,
        "LAPORAN RENCANA " . strtoupper($bidangKomisi),
        0,
        1,
        "C",
    );
    $pdf->SetFont("helvetica", "B", 12);
    $pdf->Cell(0, 10, $laporan, 0, 1, "C"); // Tambah Tabel
    ...
    SUM(CASE WHEN v.jenis COLLATE utf8mb4_unicode_ci = 'Rencana Pengeluaran' THEN v.jumlah ELSE 0 END) AS jumlah_pengeluaran
          FROM bidang b
            ...
            v.tahun = $tahun_aktif";
    $laporan = "TAHUN " . $tahun_aktif; // // echo $sql;
    if ($tahun == 0) {
        $sql .= "";
    } else {
        // $sql = "SELECT
    ...
    $sql .=
        "  GROUP BY b.id_bidang, k.id_komisi ORDER BY b.id_bidang, k.id_komisi"; // //echo $sql;
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $penerimaan_danaGereja =
            $row["penerimaan_dana_gereja"] == 0 ||
            empty($row["penerimaan_dana_gereja"])
                ? "-"
                : number_format($row["penerimaan_dana_gereja"], 0, ",", ".");
        $penerimaan_danaSwadaya =
            $row["penerimaan_dana_swadaya"] == 0 ||
            empty($row["penerimaan_dana_swadaya"])
                ? "-"
                : number_format($row["penerimaan_dana_swadaya"], 0, ",", ".");
        $penerimaan_total =
            $row["jumlah_penerimaan"] == 0 || empty($row["jumlah_penerimaan"])
                ? "-"
                : number_format($row["jumlah_penerimaan"], 0, ",", ".");
        $pengeluaran_danaGereja =
            $row["pengeluaran_dana_gereja"] == 0 ||
            empty($row["pengeluaran_dana_gereja"])
                ? "-"
                : number_format($row["pengeluaran_dana_gereja"], 0, ",", ".");
        $pengeluaran_danaSwadaya =
            $row["pengeluaran_dana_swadaya"] == 0 ||
            empty($row["pengeluaran_dana_swadaya"])
                ? "-"
                : number_format($row["pengeluaran_dana_swadaya"], 0, ",", ".");
        $pengeluaran_total =
            $row["jumlah_pengeluaran"] == 0 || empty($row["jumlah_pengeluaran"])
                ? "-"
    ...
    //             </table>';
    $tabelfooter =
        '<tr style="font-weight:bold; text-align:reight">
        ...
        '</td>
                        <td>' .
        number_format($total5, 0, ",", ".") .
        '</td>
                        <td>' .
        number_format($total6, 0, ",", ".") .
        '</td>
    ...
    $pdf->startPageGroup(); // Tambahkan halaman
    $pdf->AddPage("L"); // set auto page breaks
    $pdf->SetAutoPageBreak(true, PDF_MARGIN_BOTTOM); // Tambah Judul
    $pdf->SetFont("helvetica", "B", 16);
    $pdf->Cell(0, 10, "GEREJA KRISTEN JAWA DAYU", 0, 1, "C");
    $pdf->SetFont("helvetica", "B", 14);

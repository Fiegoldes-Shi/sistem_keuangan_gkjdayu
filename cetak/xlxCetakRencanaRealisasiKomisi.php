...
while ($row = mysqli_fetch_assoc($result)) {
    $rencana_penerimaan =
        $row["jumlah_rencana_penerimaan"] == 0 ||
        empty($row["jumlah_rencana_penerimaan"])
            ? "-"
            : number_format($row["jumlah_rencana_penerimaan"], 0, ".", ",");
    $realisasi_penerimaan =
        $row["jumlah_realisasi_penerimaan"] == 0 ||
        empty($row["jumlah_realisasi_penerimaan"])
            ? "-"
            : number_format($row["jumlah_realisasi_penerimaan"], 0, ".", ",");
    $rencana_pengeluaran =
        $row["jumlah_rencana_pengeluaran"] == 0 ||
        empty($row["jumlah_rencana_pengeluaran"])
            ? "-"
            : number_format($row["jumlah_rencana_pengeluaran"], 0, ".", ",");
    $realisasi_pengeluaran =
        $row["jumlah_realisasi_pengeluaran"] == 0 ||
        empty($row["jumlah_realisasi_pengeluaran"])
            ? "-"
            : number_format($row["jumlah_realisasi_pengeluaran"], 0, ".", ",");

    $total_rencanaPenerimaan =
        $total_rencanaPenerimaan + $row["jumlah_rencana_penerimaan"];
    $total_realisasiPenerimaan =
        $total_realisasiPenerimaan + $row["jumlah_realisasi_penerimaan"];
    $total_rencanaPengeluaran =
        $total_rencanaPengeluaran + $row["jumlah_rencana_pengeluaran"];
    $total_realisasiPengeluaran =
        $total_realisasiPengeluaran + $row["jumlah_realisasi_pengeluaran"];
...
$tabelfooter =
    '<tr style="text-align: right; font-weight:bold">
                    ...
                    <td></td>
                    <td width="12%">' .
    number_format($total_rencanaPenerimaan, 0, ".", ",") .
    '</td>
                    <td width="12%">' .
    number_format($total_realisasiPenerimaan, 0, ".", ",") .
    '</td>
                    <td width="12%">' .
    number_format($total_rencanaPengeluaran, 0, ".", ",") .
    '</td>
                    <td width="12%">' .
    number_format($total_realisasiPengeluaran, 0, ".", ",") .
    '</td>
                    </tr>
                </table>';

...
if (isset($_POST["save"])) {
    ...
    if (!empty($_FILES["bukti_penerimaan"]["name"])) {
        $uploadDir =
            realpath("../uploads/bukti_penerimaan/") . DIRECTORY_SEPARATOR;
        $fileName =
            time() . "_" . basename($_FILES["bukti_penerimaan"]["name"]);
        ...
        if (in_array($fileExt, $allowedExts)) {
            ...
            if (
                move_uploaded_file(
                    $_FILES["bukti_penerimaan"]["tmp_name"],
                    $uploadDir . $fileName,
                )
            ) {
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-12">
      <?php _myHeader("newspaper", "Penerimaan Gereja", "Entri Data"); ?>
    </div>
  </div>
  <br>
  <div class="section">
    ...
    : (isset($_SESSION["edit_data"])
        ...
        : "") ?>"
          ...
          <?php if (isset($_POST["tanggal_terima"])) {
              $_SESSION["tanggal_terima"] = $_POST["tanggal_terima"];
          } ?>
        </div>
        <div class="form-group" style="width:100%; ">
          <label for="jenis" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Jenis Penerimaan</label>
          <input style="width:90%;" type="text" id="jenis_terima" name="jenis_terima" placeholder="Masukkan Jenis Penerimaan"
            value="<?= isset($_POST["jenis_terima"])
                ? $_POST["jenis_terima"]
                : (isset($_SESSION["edit_data"])
                    ? $_SESSION["edit_data"]["jenis_terima"]
                    : "") ?>"
            required />
          <?php if (isset($_POST["jenis_terima"])) {
    ...
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      <div class="form-group" style="width:45%; margin-left: 30px;">
        ...
        <select style="width:95%" id="akun" name="akun" required>
          ...
          <?php
          $sql =
              "SELECT id_akun, nama_akun FROM akun WHERE jenis_debitKredit = 'Kredit' AND status_input = 1 ORDER BY kode_akun ASC";
...
?>
      ...
      <div class="form-group" style="width:40%; margin-left: 30px;">
        <label for="jumlah" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Jumlah Penerimaan</label>
        <input style="width:100%" type="number" id="jumlah" name="jumlah" placeholder="Masukkan Jumlah Penerimaan" value="<?= isset(
            $_SESSION["edit_data"],
        )
  ...
  <div class="section">
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      ...
      <div class="form-group" style="width:25%; margin-left: -30px;">
        <label for="bukti_penerimaan" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Bukti Penerimaan</label>
        <input style="width:95%" type="file" class="form-control" id="bukti_penerimaan" name="bukti_penerimaan" accept=".pdf, .docx, .xlsx">
      </div>

      <div class="form-group" style=" width:25%; margin-left: -30px;">
        <label for="jenis" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Keterangan</label>
        <input style="width:87%" type="text" id="deskripsi" name="deskripsi" placeholder="Masukkan Keterangan Penerimaan" value="<?= isset(
            $_SESSION["edit_data"],
        )
      ...
      <div class="form-group" style=" width:25%; margin-left: -50px; margin-right: 50px;">
        ...
        <input style="width:87%" type="text" id="penanggung_jawab" name="penanggung_jawab" placeholder="Masukkan Penanggung Jawab" value="<?= $_POST[
            "penanggung_jawab"
        ] ??
            ($_SESSION["edit_data"]["penanggung_jawab"] ?? "") ?>" required>
      </div>
    ...
    <form action="" method="post" autocomplete="off">
      <table id='data-table' class='table table-condensed table-bordered'>
        ...
        <tbody>
          ...
          $total = 0;
          if (
              isset($_SESSION["temp_data9"]) &&
              count($_SESSION["temp_data9"]) > 0
          ) {
              ...
              <tr class=''>
                ...
                <td><?= !empty($data["deskripsi"])
                    ? $data["deskripsi"]
                    : "-" ?></td>
                <td><?= !empty($data["bukti_penerimaan"])
                    ? $data["bukti_penerimaan"]
                    : "-" ?></td>

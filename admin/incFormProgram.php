...
if (isset($_POST["delete"])) {
    ...
    if (isset($_SESSION["temp_data_program"][$indexToDelete])) {
        ...
        $_SESSION["temp_data_program"] = array_values(
            $_SESSION["temp_data_program"],
        );
...
if (isset($_POST["submit_all"])) {
    if (
        isset($_SESSION["temp_data_program"]) &&
        count($_SESSION["temp_data_program"]) > 0
    ) {
...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-12">
      <?php _myHeader("newspaper", "Program", "Entri Data"); ?>
    </div>
  </div>
  ...
  <div class="section">
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      ...
      <div class="form-group" style="width:45%; margin-left: 30px; margin-bottom: 10px;">
        <label for="nama_program" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Nama Program</label>
        <input style="width:95%" type="text" id="nama_program" name="nama_program" placeholder="Masukkan Nama Program" value="<?= isset(
            $_SESSION["edit_data"],
        )
            ? $_SESSION["edit_data"]["nama_program"]
            : "" ?>" required />
      </div>
      <div class="form-group" style="width:23%; margin-left: 30px;">
        <label for="tanggal_mulai" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Tanggal Mulai</label>
        <input style="width:80%;" type="date" id="tanggal_mulai" name="tanggal_mulai" placeholder="" required min="<?= $tahun_aktif -
            1 ?>-01-01" max="<?= $tahun_aktif ?>-12-31" value="<?= isset(
    $_SESSION["edit_data"],
)
    ? $_SESSION["edit_data"]["tanggal_mulai"]
    : "" ?>" />
      ...
      <div class="form-group" style="width:23%; margin-left: -50px">
        <label for="tanggal_selesai" class="required" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Tanggal Selesai</label>
        <input style="width:80%" type="date" id="tanggal_selesai" name="tanggal_selesai" placeholder="" required min="<?= $tahun_aktif -
            1 ?>-01-01" max="<?= $tahun_aktif ?>-12-31" value="<?= isset(
    $_SESSION["edit_data"],
)
    ? $_SESSION["edit_data"]["tanggal_selesai"]
    : "" ?>" />
        <p style="margin-top: -15px; margin-left: 10px; color:#838996; font-weight: 500;">mm/dd/yyyy</p>
      </div>
    </div>
    ...
    <div class="horizontal filter-horizontal" style="display: flex; align-items: center; justify-content: flex-start; gap: 15px; flex-wrap: wrap;">
      ...
      <div class="form-group" style="width:100%; margin-left: 30px;">
        <label for="keterangan" style="white-space: nowrap; font-weight: bold; margin-bottom: 0;">Keterangan</label>
        ...
        <tbody>
          ...
          if (
              ...
              foreach ($_SESSION["temp_data_program"] as $index => $data) {
                  ...
                  $cnourut = $cnourut + 1;
                  $namaBidang = getNameFromId(
                      "bidang",
                      "id_bidang",
                      "nama_bidang",
                      $data["bidang"],
                  );
                  $namaKomisi = !empty($data["komisi"])
                      ? getNameFromId(
                          "komisi",
                          "id_komisi",
                          "nama_komisi",

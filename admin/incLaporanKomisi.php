...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <?php _myHeader(
                "document",
                "Rekapitulasi Rencana dan Realisasi Komisi",
                "Laporan",
            ); ?>
        </div>
    </div>
        ...
        <a href="../cetak/xlxCetakRencanaRealisasiKomisi.php" target="" style="background-color:#2e8b57; color:white; padding: 8px 15px; border-radius:4px; text-decoration: none; white-space: nowrap; font-weight: bold;">Cetak Excel</a>
        <a href="../cetak/pdfCetakRencanaRealisasiKomisi.php" target="_new" style="background-color:#dc143c; color:white; padding: 8px 15px; border-radius:4px; text-decoration: none; white-space: nowrap; font-weight: bold;">Cetak PDF</a>
    </div>
</div>
</div>
</div>
        ...
        if (isset($_POST["tb_tahun"])) {
            if ($_POST["tb_tahun"] == 0) {
                ...
                $sql .=
                    " WHERE fiskal.tahun =" .
                    $_POST["tb_tahun"] .
                    " ORDER BY fiskal.tahun, bidang.id_bidang, komisi.id_komisi; ";
            }
        } else {
            $sql .= " WHERE fiskal.id_fiskal = $id_fiskal ORDER BY fiskal.tahun, bidang.id_bidang, komisi.id_komisi ";
        }
            ...
            <table id="" class="table table-bordered table-condensed w-100">
                ...
                <tbody>
                    ...
                    foreach ($array as $data) {
                        ...
                        $color2 = "black";
                        if (
                            $data["jumlah_realisasi_penerimaan"] >
                            $data["jumlah_rencana_penerimaan"]
                        ) {
                            $color1 = "#008000";
                        } elseif (
                            $data["jumlah_realisasi_penerimaan"] <
                        ...
                        if (
                            $data["jumlah_realisasi_pengeluaran"] <
                            $data["jumlah_rencana_pengeluaran"]
                        ) {
                            $color2 = "#008000";
                        } elseif (
                            $data["jumlah_realisasi_pengeluaran"] >
                        ...
                        <tr>
                            ...
                            <td class="text-end" style="font-weight: bold;"><?= number_format(
                                (float) ($data["jumlah_rencana_penerimaan"] ??
                                    0),
                                0,
                                ",",
                                ".",
                            ) ?></td>
                            <td class="text-end" style="font-weight: bold; color: <?= $color1 ?>"> <?= number_format(
    (float) ($data["jumlah_realisasi_penerimaan"] ?? 0),
    0,
    ",",
    ".",
) ?></td>
                            <td class="text-end" style="font-weight: bold;"><?= number_format(
                                (float) ($data["jumlah_rencana_pengeluaran"] ??
                                    0),
                                0,
                                ",",
                                ".",
                            ) ?></td>
                            <td class="text-end" style="font-weight: bold; color: <?= $color2 ?>"><?= number_format(
    (float) ($data["jumlah_realisasi_pengeluaran"] ?? 0),
    0,
    ",",
    ".",
) ?></td>
                    ...
                    <?php
                    $total_rencanaPenerimaan +=
                        $data["jumlah_rencana_penerimaan"];
                    $total_realisasiPenerimaan +=
                        $data["jumlah_realisasi_penerimaan"];
                    $total_rencanaPengeluaran +=
                        $data["jumlah_rencana_pengeluaran"];
                    $total_realisasiPengeluaran +=
                        $data["jumlah_realisasi_pengeluaran"];
                    ...
                    if (
                        $total_realisasiPengeluaran < $total_rencanaPengeluaran
                    ) {
                        $color2 = "#008000";
                    } else {
                        $color2 = "#a52a2a";
                    ...
                    <tr>
                        ...
                        <td class="text-end" style="font-weight:bolder; color: <?= $color1 ?>"> <?= number_format(
    $total_realisasiPenerimaan,
    0,
    ",",
    ".",
                        ...
                        <td class="text-end" style="font-weight:bolder; color: <?= $color2 ?>"><?= number_format(
    $total_realisasiPengeluaran,
    0,
    ",",
    ".",
